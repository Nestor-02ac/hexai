"""
Train the Y Gumbel AlphaZero variant.

Usage:
  python train_y.py
  python train_y.py --board-size 7 --channels 128
  python train_y.py --simulations 0
  python train_y.py --iterations 5 --games-per-iter 10
"""
...
def main():
    parser = argparse.ArgumentParser(description='Train the Y Gumbel AlphaZero variant')
    ...
    parser.add_argument('--simulations', type=int, default=0,
                        help='MCTS sims per move (0 = auto: number of cells)')
    ...
    print(
        f"  Backend: {config.mcts_backend}, workers={config.num_self_play_workers}"
    )
