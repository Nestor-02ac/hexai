...
class Trainer:
    ...
    def train(self, start_iteration=0):
        for iteration in range(start_iteration, self.config.num_iterations):
            ...
            games_result = generate_self_play_data(  # unchanged if Y logic inside
                self.config,
                self.network,
                self.config.num_self_play_games_per_iteration,
                self.device,
                progress_total=self._estimate_self_play_positions(),  # relies on FIXED function
            ...
            positions = sum(len(g) for g in games)  # unchanged
    ...
    def _train_epoch(self):
        ...
        for step_idx in range(steps):
            ...
            # Safety check
            assert policy_logits.shape == target_policies.shape, \
                f"Mismatch: logits {policy_logits.shape} vs targets {target_policies.shape}"
